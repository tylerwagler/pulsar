...
set -u
ssh claude@sparky.defense.lan "pkill -x pulsar-server; sleep 3; rm -rf /home/claude/.cache/ds4/kv-v5mx4-0731-mmqaligned; cd ~/idx-engine && (env $2 PULSAR_LOCK_FILE=/home/claude/nsystmp/ds4srv.lock setsid nohup ./pulsar-server -m /srv/models/v5mx4-0731-mmqaligned.gguf --port 8099 $1 > /home/claude/nsystmp/srv_cycle.log 2>&1 < /dev/null &); sleep 30; echo up"
