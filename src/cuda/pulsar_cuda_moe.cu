...
static int routed_moe_batch_impl(pulsar_gpu_tensor *out, pulsar_gpu_tensor *up, pulsar_gpu_tensor *mid, pulsar_gpu_tensor *down, const void *model_map, uint64_t model_size, uint64_t gate_offset, uint64_t up_offset, uint64_t down_offset, uint32_t gate_type, uint32_t down_type, uint64_t gate_expert_bytes, uint64_t gate_row_bytes, uint64_t down_expert_bytes, uint64_t down_row_bytes, uint32_t expert_in_dim, uint32_t expert_mid_dim, uint32_t out_dim, const pulsar_gpu_tensor *selected, const pulsar_gpu_tensor *weights, uint32_t n_total_expert, uint32_t n_expert, float clamp, const pulsar_gpu_tensor *x, uint32_t layer_index, uint32_t n_tokens) {
    ...
    if (gate_type == 40u && down_type == 40u) {
        /* Row KIND chooses (pulsar_gpu_matmul_batch_decode_rows, pulsar_gpu.h;
         * the two-pass split above leaves the count at 0 or >= n_tokens).
         * DECODE rows (1..PULSAR_GPU_MNEUTRAL_ROWS_MAX -- the setter bounds
         * the count) take the direct fp4 GEMV over the packed expert weights:
         * 4 launches per layer (gate/up+swiglu, down, their E4M3 emits), no
         * host sync, each row's output computed from its own activation.
         * PREFILL rows take routed_moe_launch_cutlass_dispatch's grouped
         * fixed-tile projection at any n_tokens, one included.  The two do
         * NOT agree to the last bit on every row (a final-rounding difference
         * in ~1 element per 50k; L150 and L152 bisected the row-selective
         * drift of a 9..16-row drafter step to exactly this boundary when a
         * row-count cap of 8 chose here), so the choice is a numerics boundary
         * and it follows the row's kind, not the batch width (L167; until then
         * row count chose: <= 8 unarmed, <= 16 armed).  A decode call whose
         * buffers do not fit refuses -- it used to fall to the grouped path. */
        if (pulsar_gpu_matmul_batch_decode_rows() > 0) {
            if (!(mid && mid->ptr && down && down->ptr && out && out->ptr &&
                  ...
                  weights->bytes >= (uint64_t)n_tokens * n_expert * sizeof(float) &&
                  x->bytes >= (uint64_t)n_tokens * expert_in_dim * sizeof(float))) {
                fprintf(stderr, "pulsar: routed MoE small-batch FFN (decode rows, n_tok=%u): a scratch "
