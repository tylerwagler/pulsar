...
#pragma unroll
        for (int cw = 0; cw < 8; ++cw) {
            ...
            /* THE TILE'S WEIGHT NUMERICS, reproduced: make_iq2_A_tile_e4m3 rounds
             * each dequantised weight to E4M3 under the 32-block's shared
             * exponent (of amax x |d*ls*0.125|) and the MMA applies 2^se to the
             * block sum.  The first landing attempt multiplied the exact f32
             * weight and the reference gate read it +0.29% further at the one
             * decode depth; with identical weight bytes only the accumulation
             * order differs from the tile.  Magnitudes are 8/25/43, so the
             * block amax is the largest grid byte present x |dA|. */
            const float dAg = dgb * (float)((int)(cg.y >> 27) | 1) * 0.125f;
            const float dAu = dub * (float)((int)(cu.y >> 27) | 1) * 0.125f;
            uint32_t mxg = 0u, mxu = 0u;
#pragma unroll
            for (int g = 0; g < 4; ++g) {
                const uint64_t eg = s_grid[(cg.x >> (8 * g)) & 0xffu];
                const uint64_t eu = s_grid[(cu.x >> (8 * g)) & 0xffu];
                mxg = __vmaxu4(mxg, __vmaxu4((uint32_t)eg, (uint32_t)(eg >> 32)));
                mxu = __vmaxu4(mxu, __vmaxu4((uint32_t)eu, (uint32_t)(eu >> 32)));
            }
            const uint32_t amg = max(max(mxg & 0xffu, (mxg >> 8) & 0xffu), max((mxg >> 16) & 0xffu, mxg >> 24));
            const uint32_t amu = max(max(mxu & 0xffu, (mxu >> 8) & 0xffu), max((mxu >> 16) & 0xffu, mxu >> 24));
            const int seg = pulsar_mx_shared_exp((float)amg * fabsf(dAg));
            const int seu = pulsar_mx_shared_exp((float)amu * fabsf(dAu));
            const float rg = dAg * exp2f(-(float)seg);
            const float ru = dAu * exp2f(-(float)seu);
...
#pragma unroll
                for (int j = 0; j < 8; ++j) {
                    ...
                    /* the tile's byte: e4m3(mag x r), sign flipped after -- exact as a float */
                    const float qg = (float)((__nv_fp8_e4m3)((float)bg * rg));
                    const float qu = (float)((__nv_fp8_e4m3)((float)bu * ru));
                    const float mg = __uint_as_float(__float_as_uint(qg) | (((sgn_g >> j) & 1u) << 31));
                    const float mu = __uint_as_float(__float_as_uint(qu) | (((sgn_u >> j) & 1u) << 31));
                    sg = fmaf(mg, xv[j], sg);
                    su = fmaf(mu, xv[j], su);
                }
            }
            acc_g = fmaf(exp2f((float)seg), sg, acc_g);
            acc_u = fmaf(exp2f((float)seu), su, acc_u);
        }
    }
