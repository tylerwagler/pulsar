...
#include "ds4_mmq_soa_tiles.cuh"  // ds4 (L008)
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_iter) {
        ...
        // argument (its total pair count), which the upstream loader typedef
        // has no room for -- so these two types dispatch directly instead of
        // through the `load_tiles` function pointer.  x_soa == nullptr means
        // the artifact was not built for this tensor; take the stock loader.
        if (x_soa != nullptr && type == GGML_TYPE_Q2_K) {
            load_tiles_q2_K_soa<type, J, fallback>(
                x_soa, soa_blocks, tile_x, offset_x + kb0, tile_x_max_i, stride_row_x);
        } else if (x_soa != nullptr && type == GGML_TYPE_IQ2_XXS) {
            load_tiles_iq2_xxs_soa<type, J, fallback>(
                x_soa, soa_blocks, tile_x, offset_x + kb0, tile_x_max_i, stride_row_x);
        } else {
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (fixup_needed) {
        ...
        // ds4 (L008): zero the stream-k fixup buffer.  It comes from the pool
        // with whatever the last user left in it, and a partial tile leaves
        // entries the fixup pass then reads.  Measured 2026-08-14: without this
        // the prefill gate reports 129280/129280 NON-FINITE logits at depth
        // 4102 -- a full 4096 chunk followed by a 6-token tail, i.e. exactly the
        // partial-tile shape.  Multiples of the chunk size (4096/8192/...) are
        // unaffected, which is why throughput runs and multiple-of-4096 logit
        // comparisons all looked clean.  Carried since 82b2622; do not drop it
        // again on a re-sync.
        CUDA_CHECK(cudaMemsetAsync(tmp_fixup.ptr, 0,
                    (size_t)block_nums_stream_k.x * (size_t)config.J * (size_t)config.I * sizeof(float),
                    stream));
    }
