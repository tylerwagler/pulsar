...
/* The Q-upload helpers below need the CUDA runtime and pulsar_q_t; the row
 * CODEC above is pure host, so a host test can include this header without a
 * device.  Guarded rather than split so the fixture stays one file and one
 * authority.  (Restored from dev for the two-profile engine.) */
