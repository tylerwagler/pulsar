...
int main() {
    ...
    const int nblk = (K / QK8_1_MMQ) * rows;

    float *x = nullptr;
    block_q8_1_mmq *y = nullptr;
    cudaMallocManaged(&x, (size_t)rows * K * sizeof(float));
    cudaMallocManaged(&y, (size_t)nblk * sizeof(block_q8_1_mmq));
    cudaMemset(y, 0, (size_t)nblk * sizeof(block_q8_1_mmq));
    ...
    for (int r = 0; r < rows; ++r) {
        for (int k = 0; k < K; ++k) {
            const int ib   = (k / QK8_1_MMQ) * rows + r;
            const int iqs  = k % QK8_1_MMQ;
    ...
    if (wr >= 0) {
        const int ib  = (wk / QK8_1_MMQ) * rows + wr;
        const int iqs = wk % QK8_1_MMQ;
